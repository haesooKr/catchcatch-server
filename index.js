...
io.on("connection", socket => {
  ...
  socket.on("create", ({ nick, color, round, timer, language }, callback) => {
    ...
    return callback({
      code: `http://localhost:3000/join?code=${rot13(socket.id)}`
    });
  });
  ...
  socket.on("correct", callback => {
    ...
    if (
      getUsersInRoom(user.room).filter(user => user.point[0] === false).length <
      1
    ) {
  ...
  socket.on("disconnect", () => {
    ...
    if (user !== undefined) {
      if (
        ...
        user.id === getUsersInRoom(user.room)[0].id
      ) {
        socket.broadcast.to(user.room).emit("message", {
          user: "admin",
          text:
            "Host has left before the game starts. If you cannot see INVITE CODE in this room, please leave the room, and create a new room to play."
        });
      }
      if (user.room.start === true && user.room.turn === user.id) {
        ...
        socket.broadcast.to(user.room).emit("next", {
          // ***
          timer: user.room.timer,
          turn: user.room.turn,
          points: getUsersInRoom(user.room)
            .filter(user => user.id !== id)
            .map(user => [user.point[1]]),
          words: randomWords(),
          roundTurn: user.room.turn === getUsersInRoom(user.room)[0].id // check last player's turn
        });
      }
      ...
      if (getUsersInRoom(user.room).length > 1) {
        socket.broadcast
          .to(user.room)
          .emit("online", getUsersInRoom(user.room));
      } else {
        socket.broadcast
          .to(user.room)
          .emit("online", getUsersInRoom(user.room), true);
      }
